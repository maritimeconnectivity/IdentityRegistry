...
public class VesselController {
    ...
    @RequestMapping(
            ...
            method = RequestMethod.GET,
            produces = "application/json;charset=UTF-8")
    public ResponseEntity<?> newOrgCert(HttpServletRequest request, @PathVariable String orgShortName, @PathVariable Long vesselId) {
        Organization org = this.organizationService.getOrganizationByShortName(orgShortName);
        if (org != null) {
            // Check that the user has the needed rights
            if (AccessControlUtil.hasAccessToOrg(org.getName(), orgShortName)) {
                ...
                if (vessel.getIdOrganization() == org.getId().intValue()) {
                    ...
                    X509Certificate vesselCert = CertificateUtil.generateCertForEntity(org.getCountry(), org.getName(), vessel.getName(), vessel.getName(), "", vesselKeyPair.getPublic());
